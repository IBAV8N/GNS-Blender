...
DR_dh_test		= find_dataref("laminar/c90/avionics/switch_pos/dh_test")
DR_efis_horn	= find_dataref("laminar/c90/warning/EFIS/data/aux_horn_on")
...
simDR_gpws = find_dataref("sim/cockpit/weapons/firing_mode")
-- simDR_gpws = 1
...
function initialize()
    messages = {
        ...
        [28] = { id = 'bank_angle', is_playing = false, wants_play = false, duration = 1800 }, -- TODO
...
function dh_test()
    if DR_dh_test == 1 then
        play_message('minimums', true)
        is_minimums_played = false --Edit by IBAV8N
        --dh_test = 1 --These crash the function
	--elseif DR_dh_test == 0 and dh_test == 1 then
		--simDR_gpws = 0
		--dh_test = 0
	end
end
...
function mode_5_glideslope()
    if DR_gs_annun == 1 then
        ...
        simDR_gpws = 1
        gs_annun = 1
	elseif DR_gs_annun == 0 and gs_annun == 1 then
		simDR_gpws = 0
        gs_annun = 0
	end
end
...
function mode_1_pull_up()
    if DR_pull_up_annun == 1 and DR_vario < 0 then
        ...
        simDR_gpws = 1
        pull_up_annun = 1
	elseif DR_pull_up_annun == 0 and pull_up_annun == 1 then
		simDR_gpws = 0
		pull_up_annun = 0
	end
end
...
function mode_6_bank_angle()
    ...
    if DR_autopilot_on == 0 then
        ...
        if DR_rad_alt > 10 and DR_rad_alt < 210 then
            if roll_angle >= 30 then -- TODO: Use a curve instead of a fixed limit
	        ...
	        elseif roll_angle < 30 and bank_angle == 1 then
		        simDR_gpws = 0
		        bank_angle = 0
            end
        elseif DR_rad_alt >= 210 then
            if roll_angle >= 50 then
                if not is_bank_angle_played then
                    play_message('bank_angle', true)
                    is_bank_angle_played = true
                    simDR_gpws = 1
	        ...
	        elseif roll_angle < 50 and bank_angle == 1 then
		        simDR_gpws = 0
		        bank_angle = 0
            end
        end
    ...
    elseif DR_autopilot_on == 1 then
        ...
        if DR_rad_alt > 10 and DR_rad_alt < 156 then
            if roll_angle >= 20 then
                if not is_bank_angle_played then
                    play_message('bank_angle', true)
                    is_bank_angle_played = true
                    simDR_gpws = 1
	        ...
	        elseif roll_angle < 20 and bank_angle == 1 then
		        simDR_gpws = 0
		        bank_angle = 0
            end
        elseif DR_rad_alt >= 156 then
	        ...
	        elseif roll_angle < 33 and bank_angle == 1 then
		        simDR_gpws = 0
		        bank_angle = 0
            end
        end
    ...
    if is_bank_angle_played and roll_angle < 15 then
        is_bank_angle_played = false
    end
end
...
function mode_1_sinkrate()
    ...
    if DR_rad_alt < 4500 and DR_vario < -1500 and DR_rad_alt < min_alt then
...
function mode_4_too_low()
    ...
    if DR_gear_ratio < 1 and DR_on_ground == 0 then
        ...
        if DR_airspeed < 178 and DR_rad_alt < 500 and DR_vario < -300 then
        ...
        if DR_airspeed >= 178 and DR_rad_alt < 750 and DR_rad_alt < min_alt and DR_vario < -300 then
    ...
    -- Mode 4b
    elseif DR_gear_ratio == 1 and DR_flap_ratio < 0.5 and DR_on_ground == 0 and DR_vario < -300 then

        if DR_airspeed < 120 and DR_rad_alt < 170 then
        ...
        if DR_airspeed >= 120 and DR_rad_alt < 750 and DR_rad_alt < min_alt and DR_vario < -300 then
...
function pws_windshear()
    if DR_windshear_annun == 1 then
        ...
        simDR_gpws = 1
        windshear = 1
    elseif DR_windshear_annun == 0 and windshear == 1 then
	    simDR_gpws = 0
        windshear = 0
    end
end
